...
def build_prompt(form: dict) -> str:
    """
    Subject-agnostic PBL project generator, formal handout style (any subject).
    """
    title        = form.get("title", "")
    subject      = form.get("subject", "")
    grade        = form.get("grade", "Any")
    duration     = form.get("duration", "2")
    group_size   = form.get("group_size", "3–4")
    driving_q    = form.get("driving_q", f"How can we learn key ideas in {subject or 'this subject'} through a real-world project?")
...
Output **Markdown** only (no code fences, no placeholder text).
...
## Milestones & Timeline
A week-by-week table (Week, Focus, Key Tasks, Interim Deliverables) for {duration} weeks.

## Collaboration & Roles
Recommended roles (e.g., Research Lead, Designer/Builder, Analyst, Communicator) and team norms.

## Inquiry & Research Plan
Guiding questions; suggested sources; data collection or experimental design (adapt to {subject}).

## Requirements & Assumptions
Concrete requirements students must meet and assumptions they should state.

## Scaffolding & UDL Supports
Differentiation, accessibility, language supports, checklists, exemplars.

## Assessment & Rubrics
Criteria for content mastery + 21st-century skills. Provide a compact rubric table.
...
def generate():
    ...
    try:
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Answer in Markdown only. Avoid placeholders."},
                {"role": "user", "content": prompt},
            ],
            max_completion_tokens=1400                      # GPT-5 family param
            # Do not pass temperature/top_p for GPT-5 mini on Azure
        )

        # Robust content extraction
        choice = resp.choices[0]
        md_text = getattr(choice.message, "content", None)
        if not md_text and isinstance(choice.message, dict):
            md_text = choice.message.get("content")

        if not md_text or not md_text.strip():
            return jsonify({"ok": False, "error": "Model returned empty content."}), 502

        # --- Debug footer inside the PDF ---
        footer = f"""

---

<div class="muted">
Generated by deployment: <b>{DEPLOYMENT}</b><br/>
API version: <b>{API_VER}</b>
</div>
"""
        md_text += footer

        # And log to server/Log Stream
        print(f"[DEBUG] Generated PDF using deployment={DEPLOYMENT}, api_version={API_VER}")
